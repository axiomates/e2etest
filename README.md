...
<root>/                                      # 数据根目录
  ...
  .locks/                                    # 测试用例跨进程读写锁目录
    desktop.lock                             # 当前数据根目录录制/回放期间的桌面输入互斥锁
    <name>.lock                              # 仅操作期间存在，操作结束后自动删除
  .staging/                                  # 录制提交前的临时目录，成功或失败后清理
  .trash/                                    # 删除用例时的临时中转目录，删除完成后清理
```

每个名称对应一个测试用例。重录同名用例前必须先删除，不会创建版本历史。`.locks` 目录可能保留，但其中的 `desktop.lock` 和 `<name>.lock` 文件会在对应操作结束后删除。桌面输入锁只在当前数据根目录内生效，不读取、创建或检查 `%LocalAppData%` 或其他 root 外目录；不同 root 彼此独立。`.staging` 和 `.trash` 只在操作过程中短暂出现。
...
| `ai.maxOutputTokens` | `12000` | OpenAI Chat Completions 请求的 `max_tokens`；限制单次 AI 复核的最大输出长度，必须大于 `0` |
...
`baseUrl` 是 OpenAI 兼容接口的基址（不要加 `/chat/completions`）；`model` 使用服务商给出的模型 ID。`contextPrompt` 是可选的项目背景，可描述被测软件、业务术语、允许的动态变化和必须关注的错误；通用 prompt 不再默认放行文件数量、时间或 3D 数值变化，项目确实允许时应在这里明确说明。不要在其中放 API key 等秘密。`enableThinking` 可设为 `true` 或 `false`，用于支持该字段的模型；服务不支持 `enable_thinking` 时应删除 `enableThinking` 或设为 `null`，请求中便不会出现该字段。`maxImageDimension` 是上传图片和动态四宫格的最长边，`0` 表示全图不缩放；四宫格在 `0` 时仍以 1080 为生成预算，避免无界拼图。`maxEvidenceRegions` 默认是 `10`，证据先覆盖不同步骤，再用差异像素较大的区域补满。prompt 会同时告知 AI 本 testcase 发现的全部 rect 差异数量、已附区域以及未附图区域的 ID。`maxOutputTokens` 默认 `12000`，会作为请求体的 `max_tokens`，只限制模型输出，不改变输入上下文容量。`maxAttempts` 默认 `3`，只对网络连接失败、408、429 和 5xx 重试；`retryDelayMs` 默认 `1000`，按指数退避且单次最多等待 10 秒；`timeoutMs` 是包含重试在内的总超时。基准图、实际图的本地像素比较始终使用原始尺寸，不受这些 AI 传输参数影响。

运行 `publish.ps1` 时，如果仓库根目录存在被 Git 忽略的本地 `config.json`，脚本会在三个 EXE 发布完成后把它复制到同一发布目录。这样发布包可直接使用本机配置，但 API Key 不会进入 Git 历史；对外分发前应检查或删除该配置。
